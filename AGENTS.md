...
### 3.1 页面骨架（所有页统一）
页面标题（页名 + 一句话说明）→ 配置区（卡片化）→ **保存**。
- **默认范式：自动保存**——修改后停止操作约 1.5 秒自动写入设备（防抖合并、同值不触发，见 `AppViewModel._saveDebounce`/`AutoSaveDelayMs`）。页面底部常驻轻量保存状态行（`待保存…` → `正在保存…` → `已保存 ✓`）；**自动保存成功不弹 Toast**，避免"改一下弹一下"的噪音（失败仍弹 Toast，见 3.3）。
- **例外：快捷指令（宏）页手动保存**——宏内容不直接写入鼠标（官方驱动同样不支持独立宏写入，宏经「改键设置」绑定按键后由设备按绑定生效），保存仅落本地 `macros.json`，用户期望显式确认。该页保留显式「保存」按钮（`SaveMacroCmd`），关闭编辑（`MacroPage.xaml.cs CloseMacro_Click`）或切页导航（`MainWindow.Navigate`）时若 `SaveStatusText == "未保存"` 弹二次确认（保存并关闭 / 丢弃 / 取消）。其余自动保存页不提供操作区保存按钮。

### 3.2 语言与文案
- 用**用户语言**，禁止暴露协议术语：不出现「0x08」「entry」「校验和」「Feature Report」「HID」「字节」「原始数据」等。
  全站不提供任何原始字节 / 字节级展示（含进阶折叠区）。
- 错误提示遵循**「发生了什么 + 你可以怎么做」**结构，禁止只给报错码或技术原因。分场景规范（对应 `AppViewModel` 真实分支）：
  - 官方驱动占用 → 「检测到官方驱动运行中，写入已拦截。请完全退出 Mouse.exe 后重试。」
  - 数值非法（DPI 等） → 「第 N 档数值无效：请输入 40-4800 且为 40 的倍数。」
  - 设备已达上限 → 「设备最多支持 6 个快捷指令，请先删除一个。」
  - 文件无法识别（导入） → 「文件无法识别，请检查是否本软件导出。」
  - 保存失败兜底 → 「保存失败：请确认已退出官方驱动后重试。」（绝不出现 0x57 等内部码）
- 同一操作全站同一动词：统一「保存」「恢复出厂」，不混用「应用/加载/提交/写入」等词。

### 3.3 反馈（分层，避免噪音）
- **自动保存页**（DPI / 按键 / 灯光 / 性能等）：用页面底部**轻量状态行**反馈生命周期——`待保存…`（修改未保存高亮）→ `正在保存…`（写入中）→ `已保存 ✓`；**成功不弹 Toast**。禁止「改了没反应」：任何修改都要即时进入可见的「待保存…」状态。仅当校验失败（如数值非法）或写设备失败时才弹 Toast 告知原因。
- **手动保存 / 危险操作**（快捷指令「保存」、恢复出厂、本地配置切换、电源修改）：保存/写入期间显示**进度或禁用态**，完成后给**成功 Toast**；失败按 3.2 给「怎么办」文案。
- 所有异步操作（连接、写入、宏分块、本地配置全量切换）必须显示进度或禁用态，禁止「点了没反应」。

### 3.4 防呆与可逆
- **危险操作**（0x08 整表覆写、电源命令、重置、删除槽位）必须二次确认。
...
- **禁用 `TextButtonStyle`**：`Themes/Styles.xaml` 中的 `TextButtonStyle` 是原生 WPF 按钮样式、无自定义模板，会露出系统默认 chrome（灰色底 + 直角描边），与自研 `PrimaryButtonStyle`/`SecondaryButtonStyle` 并排时风格割裂。**任何 XAML 一律禁止引用 `TextButtonStyle`**；所有按钮改用 `PrimaryButtonStyle`（主操作/品牌色）或 `SecondaryButtonStyle`（次操作/标准浅填充圆角按钮）。若存在历史引用需清理替换。
