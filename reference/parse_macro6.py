#!/usr/bin/env python3
"""Parse macro_1.pcapng - parse USBPcap header correctly"""
import struct
...
while pos < len(data):
    if pos + 16 > len(data):
        break
    ts_sec, ts_usec, incl_len, _ = struct.unpack_from('<IIII', data, pos)
    pos += 16
    ...
    # bytes 0-1: headerLen (USHORT)
    # bytes 2-9: irpId (UINT64)
    # bytes 10-13: status (ULONG)
    # bytes 14-15: function (USHORT)
    # byte 16: info
    # byte 17: bus
    # byte 18: device
    # byte 19: endpoint
    ...
    is_control = (urb_function in [0x0008, 0x001B])
    if not is_control:
        continue
    
    # USB data starts after header
...
for i, xfer in enumerate(control_xfers):
    ...
    if rid == 0x08:
        ...
        for j in range(4, len(d)-1, 3):
            ...
            e = d[j:j+3]
            if e[0] == 0x12:
                print(f"    → 宏参数: p1=0x{e[1]:02X}  p2=0x{e[2]:02X}")
                print(f"      p1可能是播放方式, p2可能是宏索引")
